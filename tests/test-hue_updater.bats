...
@test "should turn off hue light on JENKINS_DOWN" {
    run $hue_updater JENKINS_DOWN "$lightStateUrl"

    [ "$output" == "Calling curl with -X PUT -d {\"on\":false} http://example.com/lights/1/state" ]
}

@test "should update hue with green on GOOD" {
    run $hue_updater GOOD "$lightStateUrl"

    [ "$output" == "Calling curl with -X PUT -d {\"hue\":25717,\"on\":true,\"bri\":255,\"sat\":255} http://example.com/lights/1/state" ]
}

@test "should update hue with red on BAD" {
    run $hue_updater BAD "$lightStateUrl"

    [ "$output" == "Calling curl with -X PUT -d {\"hue\":0,\"on\":true,\"bri\":255,\"sat\":255} http://example.com/lights/1/state" ]
}

@test "should update hue to custom color on BAD" {
    run $hue_updater --bad-color 12345 BAD "$lightStateUrl"

    [ "$output" == "Calling curl with -X PUT -d {\"hue\":12345,\"on\":true,\"bri\":255,\"sat\":255} http://example.com/lights/1/state" ]
}
...
@test "should update saturation to custom value" {
    ...
    [ "$output" == "Calling curl with -X PUT -d {\"hue\":0,\"on\":true,\"bri\":255,\"sat\":111} http://example.com/lights/1/state" ]
}

@test "should update brightness to custom value" {
    run $hue_updater --brightness 111 BAD "$lightStateUrl"

    [ "$output" == "Calling curl with -X PUT -d {\"hue\":0,\"on\":true,\"bri\":111,\"sat\":255} http://example.com/lights/1/state" ]
}
